...
int write_wave_file(char* filename,short * wavebuf,int size)
{
	FILE * f;
	wav_hdr wavhdr;
		...
		if( (wg->start_delay_cnt > wg->start_delay) && !wg->all_data_sent )
		{
			...
			if( wg->cur_bit_time >= wg->bit_time )
			{
				if(wg->tx_buffer_offset >= wg->tx_buffer_size)
				{
					...
					{
						wg->tx_buffer_size = prepare_next_word((int*)&wg->tx_buffer,get_byte(fc,wg->data_buffer_offset, NULL), 0, 1, 7, 1, 1, 1, 1);
...
int main(int argc, char* argv[])
{
	...
	printf("Minitel VDT to WAV converter v0.1.0.1\n");
	...
	wg.start_delay = wg.sample_rate * 2;
	...
	if(isOption(argc,argv,"wave",(char*)&ofilename)>0)
	{
		...
		if(open_file(&fc, filename,0xFF)>=0)
		{
			do
			{
				...
				write_wave_file(ofilename,(short*)&wavbuf,size);
